...
def split_msg(msg):
    ...
    msg_list.append(" ".join(msg.split()[:msg_len//3]))
    msg_list.append(" ".join(msg.split()[msg_len//3:2*msg_len//3]))
    msg_list.append(" ".join(msg.split()[2*msg_len//3:]))
    msg_size = len(max(msg_list, key=len))
    return msg_list, msg_size

def douche_says(douche, phrase):
    line_counting = 0
    msg_list, msg_len = split_msg(phrase)
    #print(dinamic_split_msg(msg))
    for line in douche.splitlines():
        line_counting += 1
        if line_counting == 4:
            i = 0
            print(f"{line}    ", end='')
            ...
            print(f"{line}   |  {msg_list[0]}", end=' ')
            for i in range(msg_len+1-len(msg_list[0])):
                print(" ", end='')
            print("|")
        elif line_counting == 6:
            print(f"{line}   |  {msg_list[1]}", end=' ')
            for i in range(msg_len+1-len(msg_list[1])):
                print(" ", end='')
            print("|")
        elif line_counting == 7:
            print(f"{line}   |  {msg_list[2]}", end=' ')
            for i in range(msg_len+1-len(msg_list[2])):
                print(" ", end='')
            print("|")
