...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ]; then
        echo "hyprshade_filter=\"$choice\"" >~/.config/hyprscripts/hyprshade_config.sh 
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    if [ -f ~/.config/hyprscripts/hyprshade_config.sh ]; then
        source ~/.config/hyprscripts/hyprshade_config.sh
    fi
